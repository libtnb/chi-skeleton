...
type Session struct {
	// Lifetime is how long a session stays valid, in minutes.
	Lifetime int `koanf:"lifetime"`
	// GcInterval is how often expired sessions are collected, in minutes.
	GcInterval int `koanf:"gc_interval"`
}
...
func (c *Config) fillDefaults() {
	// zero would make RequestSize reject every body
	if c.HTTP.BodyLimit <= 0 {
		c.HTTP.BodyLimit = 4096
	}
	if c.Session.Lifetime <= 0 {
		c.Session.Lifetime = 120
	}
	if c.Session.GcInterval <= 0 {
		c.Session.GcInterval = 30
	}
	if c.Log.Level == "" {
		c.Log.Level = "info"
	}
