...
- **Database-backed sessions** ([sessions](https://github.com/libtnb/sessions) + gorm store), started for every request (health probes excluded)
...
- **Request binding + validation** ([chix](https://github.com/libtnb/chix) + [validator](https://github.com/libtnb/validator)) with a boolean rule DSL, i18n messages and custom database rules ([gormrules](https://github.com/libtnb/validator/tree/main/contrib/gormrules))
...
Put your built frontend under `web/` and serve it from the router in `bootstrap/http.go`:
